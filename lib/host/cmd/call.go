...
func (c *CallCmd) Run() error {
	...
	if len(splitParam) == 1 {
		if c.Package.Type != data.PackageTypeInternal {
			log.Panic("no plugin CALL by not Internal Package")
		}
	...
	for i, v := range splitAction {
		...
		cmd, err := ToCmd(c.Package, plugin, split[0], param, c.ctx)
		if err != nil {
			return fmt.Errorf("action %s:%s:%d: %w", plugin.Name, actionName, i, err)
		}
		err = cmd.Run()
		if err != nil {
